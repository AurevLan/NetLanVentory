...
- **#1 Attack path graph engine** (`netlanventory/core/attack_paths.py`, migration 0053) — pure-Python depth-bounded DFS reachability graph (no networkx dependency) with V1 adjacency by `/24`. Persists top-N critical chains `(source → target, hops, total_weight)`. Endpoints: `GET /attack-paths/critical`, `GET /attack-paths/asset/{id}/{inbound,outbound}`, `POST /attack-paths/refresh`.
