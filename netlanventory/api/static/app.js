...
async function loadAssets() {
  ...
  try {
    ...
    if (!filtered.length) {
      tbody.innerHTML = '<tr><td colspan="11" style="text-align:center;color:var(--text-muted);padding:32px">No assets found</td></tr>';
    ...
    tbody.innerHTML = filtered.map(a => {
      ...
      const cveBadge = unacked.length
        ? `<span class="badge badge-${critCount(a) > 0 ? 'error' : 'warning'}">${unacked.length}</span>`
...
async function openAssetModal(id) {
  ...
  try {
    ...
    // SSH Profile dropdown (admin only — gracefully hidden for non-admins)
    ...
    // SSH auto-scan per-asset settings
    ...
    // Trivy auto-scan per-asset settings
    ...
    // ── Trivy Docker section ────────────────────────────────────────────
    let trivyReports = [];
    try {
      trivyReports = await api(`/assets/${id}/trivy-docker`) || [];
    } catch (_) { /* Trivy reports might not exist yet */ }
...
async function loadSshProfiles() {
  try {
    ...
    if (!_sshProfiles.length) {
      tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:32px">No SSH profiles yet</td></tr>';
      return;
    }
...
async function loadDashboard() {
    ...
    _dashSeverityChart = new Chart(sevCtx, {
      ...
      options: {
        ...
        plugins: {
          legend: {
            position: 'right',
            labels: { color: TICK_COLOR, font: CHART_FONT, padding: 14, boxWidth: 12, borderRadius: 4 },
          },
          tooltip: {
            backgroundColor: 'rgba(15,18,30,0.92)',
            titleColor: '#e2e8f0',
  ...
  // Horizontal bar: top 5 vulnerable assets
  ...
  // Render action required cards and load trends
...
function _renderExpositions() {
  ...
  const ack      = document.getElementById('exp-ack')?.value;  // '' = all, 'none' = unack only
  ...
  let filtered = _expData.filter(({ asset, cve }) => {
    ...
    if (search) {
      const hay = [
        cve.cve_id_str, asset.name, asset.ip, asset.hostname,
  ...
  if (!page.length) {
    tbody.innerHTML = '<tr><td colspan="10" style="color:var(--text-muted);text-align:center;padding:24px">Aucune exposition trouvée.</td></tr>';
...
async function loadCves(reset = false) {
  ...
  tbody.innerHTML = '<tr><td colspan="7" style="color:var(--text-muted);text-align:center;padding:24px">Chargement…</td></tr>';

  let data;
  try {
    data = await api(`/cves?${params}`);
  } catch (e) {
    tbody.innerHTML = `<tr><td colspan="7" style="color:var(--danger);text-align:center;padding:24px">Erreur : ${escape(String(e))}</td></tr>`;
    return;
  }
  ...
  if (!data.items.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="color:var(--text-muted);text-align:center;padding:24px">Aucun CVE en base.</td></tr>';
  ...
  tbody.innerHTML = data.items.map(c => {
    ...
    const planCell = c.remediation
      ? `<span class="plan-snippet clickable" title="${escape(c.remediation)}"
             ...
             >${escape(c.remediation.length > 60 ? c.remediation.slice(0, 60) + '…' : c.remediation)}</span>`
      : (isAdmin
    ...
    return `
      <tr>
        ...
        <td style="font-size:12px;color:var(--text-muted);max-width:280px" title="${escape(c.description || '')}">${desc}</td>
        <td style="max-width:200px;font-size:12px">${planCell}</td>
...
// ── loadAssets — replacement with all features (#3, #4, #5) ──────────────────
// This declaration comes after the original one in the file; the last wins.
...
async function loadAssets() {
  ...
  try {
    ...
    if (!filtered.length) {
      tbody.innerHTML = '<tr><td colspan="13" style="text-align:center;color:var(--text-muted);padding:32px">No assets found</td></tr>';
      if (bulkBtn) bulkBtn.style.display = 'none';
    ...
    if (_groupBySubnet) {
      ...
      Object.entries(groups).sort().forEach(([subnet, assets]) => {
        rows += `<tr class="subnet-header"><td colspan="13">${escape(subnet)} — ${assets.length} host${assets.length > 1 ? 's' : ''}</td></tr>`;
...
// ── _renderExpositions — replacement with checkboxes (#9) ──────────────────────
...
function _renderExpositions() {
  ...
  if (!page.length) {
    tbody.innerHTML = '<tr><td colspan="11" style="color:var(--text-muted);text-align:center;padding:24px">Aucune exposition trouvée.</td></tr>';
    _renderExpPagination(total);
    return;
  }
...
// ── loadCves — replacement with clickable affected count (#10) ────────────────
  ...
  tbody.innerHTML = data.items.map(c => {
    ...
    const planCell = c.remediation
      ...
      : (isAdmin
          ? `<button class="btn btn-sm" onclick="openCveRemediationModal('${escape(c.cve_id)}','')">+ Plan</button>`
          : '<span style="color:var(--text-muted);font-size:11px">—</span>');
    // Clickable affected count
    const affectedCell = c.asset_count > 0
      ? `<span class="clickable" style="font-weight:600;color:var(--danger);cursor:pointer"
             onclick="showCveAffectedAssets('${escape(c.cve_id)}',${c.asset_count})"
             title="Voir les assets affectés">${c.asset_count}</span>`
      : '0';
    return `
      <tr>
        <td><a class="cve-link" href="${cveUrl(c.cve_id)}"
...
// ── initSubTabs — patched to handle quota tab (#18) ───────────────────────────
...
// ── _initAppData — patched to wire new features ────────────────────────────────
...
async function loadReportsPanel() {
  ...
  try {
    ...
    if (!data) return;
    _renderReportsStats(data);
    // API returns top_risky_assets (array) — sla_breaches is a count, not a list
    _renderReportsSlaBreaches([]);
    _renderReportsTopRisk(data.top_risky_assets || data.top_risk_assets || []);
    // Trend from dashboard/trends endpoint
    try {
      const trend = await api('/dashboard/trends');
      if (trend) _renderReportsTrend(trend);
    } catch (_) { /* trend indisponible */ }
  } catch (e) {
    const el = document.getElementById('reports-stats');
    if (el) el.innerHTML = `<span style="color:var(--danger);font-size:13px">Erreur : ${escape(e.message)}</span>`;
    if (slaEl) slaEl.innerHTML = '<p style="color:var(--text-muted);font-size:13px">Indisponible.</p>';
    if (riskEl) riskEl.innerHTML = '<p style="color:var(--text-muted);font-size:13px">Indisponible.</p>';
  }
}
...
async function loadTopology() {
  ...
  // Wait up to 3s for D3 to load from CDN
  let d3Attempts = 0;
  while (!window.d3 && d3Attempts < 15) {
    await new Promise(r => setTimeout(r, 200));
    d3Attempts++;
  }
  if (!window.d3) {
    container.innerHTML = '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:10px">'
      + '<span style="color:var(--text-muted);font-size:13px">D3.js n\'a pas pu être chargé depuis le CDN.</span>'
      + '<span style="color:var(--text-muted);font-size:11px">Vérifiez votre connexion internet ou les paramètres CSP.</span></div>';
    return;
  }

  try {
    const resp = await api('/assets?limit=500');
    _topoAssets = (resp && resp.items) ? resp.items : (Array.isArray(resp) ? resp : []);
    _renderTopology(_topoAssets);
  } catch (e) {
    container.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--danger);font-size:13px">Erreur chargement assets : ${escape(e.message)}</div>`;
  }
}
...
// ── loadCves — adds EPSS column and filter ────────────────────────────────────
...
async function loadCves(reset = false) {
  ...
  const tbody = document.getElementById('cve-global-tbody');
  tbody.innerHTML = '<tr><td colspan="8" style="color:var(--text-muted);text-align:center;padding:24px">Chargement…</td></tr>';

  let data;
  try {
    data = await api(`/cves?${params}`);
  } catch (e) {
    tbody.innerHTML = `<tr><td colspan="8" style="color:var(--danger);text-align:center;padding:24px">Erreur : ${escape(String(e))}</td></tr>`;
    return;
  }
  ...
  if (!data.items.length) {
    tbody.innerHTML = '<tr><td colspan="8" style="color:var(--text-muted);text-align:center;padding:24px">Aucun CVE en base.</td></tr>';
    _renderCvePagination(data.total);
    return;
  }
